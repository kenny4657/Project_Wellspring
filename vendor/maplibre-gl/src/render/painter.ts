...
export class Painter {
    ...
    clearStencil() {
        ...
        this._landClipMaskRendered = false;
    ...
    _renderTileMasks(tileStencilRefs: {[_: string]: number}, tileIDs: Array<OverscaledTileID>, renderToTexture: boolean, useBorders: boolean) {
        ...
        for (const tileID of tileIDs) {
            ...
            program.draw(context, gl.TRIANGLES, DepthMode.disabled,
                // Tests will always pass, and ref value will be written to stencil buffer (bits 0-6 only).
    ...
    /**
     * Renders the fill geometry of the specified source into bit 7 of the stencil buffer.
     * This creates a land clip mask — fill layers with clipToSource set will only render
     * where bit 7 is set (inside the land polygons).
     */
    renderLandClipMask(clipSourceId: string) {
        if (this._landClipMaskRendered) return;
        this._landClipMaskRendered = true;

        const context = this.context;
        const gl = context.gl;
        const transform = this.transform;
        const tileManagers = this.style.tileManagers;
        const tileManager = tileManagers[clipSourceId];
        if (!tileManager) return;

        const coords = this._coordsAscending[clipSourceId];
        if (!coords || !coords.length) return;

        context.setColorMode(ColorMode.disabled);
        context.setDepthMode(DepthMode.disabled);

        // Find the fill layer that uses this source (to access its bucket data)
        let clipLayerId: string | undefined;
        for (const id of this.style._order) {
            const layer = this.style._layers[id];
            if (layer.source === clipSourceId && layer.type === 'fill') {
                clipLayerId = id;
                break;
            }
        }
        if (!clipLayerId) return;
        const clipLayer = this.style._layers[clipLayerId];
        ...
        for (const coord of coords) {
            ...
            const bucket = tile.getBucket(clipLayer) as any;
            if (!bucket?.layoutVertexBuffer) continue;

            const terrainData = this.style.map.terrain && this.style.map.terrain.getTerrainData(coord);
    ...
    stencilModeForClipping(tileID: OverscaledTileID): StencilMode {
        const gl = this.context.gl;
        // Test only bits 0-6 for tile clipping (bit 7 reserved for land clip mask)
        return new StencilMode({func: gl.EQUAL, mask: 0x7F}, this._tileClippingMaskIDs[tileID.key], 0x00, gl.KEEP, gl.KEEP, gl.REPLACE);
    }

    /**
     * Returns a stencil mode that writes bit 7 while also doing normal tile clipping.
     * Used by clip mask source layers (e.g., land-fill) to mark land pixels.
     */
    stencilModeForClipMaskWrite(_tileID: OverscaledTileID): StencilMode {
        const gl = this.context.gl;
        // Unconditionally write bit 7 on every pixel the fill renders.
        // No tile ref test — avoids tile boundary issues where refs don't match.
        return new StencilMode({func: gl.ALWAYS, mask: 0}, 0x80, 0x80, gl.KEEP, gl.KEEP, gl.REPLACE);
    }

    /**
     * Returns a stencil mode that tests both tile clipping (bits 0-6) AND land clip mask (bit 7).
     * Pixels must be inside both the tile boundary and the land polygon to pass.
     */
    stencilModeForClippingWithLandMask(tileID: OverscaledTileID): StencilMode {
        const gl = this.context.gl;
        const tileRef = this._tileClippingMaskIDs[tileID.key];
        // Test: (stencil & 0xFF) == (tileRef | 0x80)
        // This requires both the tile ref bits (0-6) to match AND bit 7 to be set (land mask)
    ...
    render(style: Style, options: PainterOptions) {
        ...
        // Store for use by renderLandClipMask (called from draw_fill)
        this._coordsAscending = coordsAscending;

...
export function drawFill(painter: Painter, tileManager: TileManager, layer: FillStyleLayer, coords: Array<OverscaledTileID>, renderOptions: RenderOptions) {
    ...
    if (opacity.constantOr(1) === 0 && !layer.isClipMaskSource) {
        return;
    }
...
function drawFillTiles(
    ...
    for (const coord of coords) {
        ...
        if (layer.isClipMaskSource) {
            // Write stencil bit 7 as this layer renders — marks land pixels for clip consumers
            stencil = painter.stencilModeForClipMaskWrite(coord);
        } else if (layer.clipToSource) {
            // Only render where bit 7 is set (inside land) AND tile clipping passes

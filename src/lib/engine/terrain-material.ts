/**
 * Terrain material — extends Babylon's StandardMaterial with custom
 * vertex displacement and fragment color logic via CustomMaterial.
 *
 * This gives us Babylon's full lighting pipeline (all scene lights work)
 * PLUS custom terrain effects (noise displacement, edge blending).
 */
...
vec4 pm(vec4 x){return mr4(((x*34.)+1.)*x);}
...
export function createTerrainMaterial(scene: Scene): CustomMaterial {
	...
	mat.Vertex_Definitions(`
		attribute vec4 terrainData0; // [type, n0, n1, n2]
		attribute vec4 terrainData1; // [n3, n4, n5, pad]
		varying vec2 vHexUV;
	...
	// Override diffuse color with terrain color + edge blending
	mat.Fragment_Custom_Diffuse(`
		vec3 tc[${N}]; initTC(tc);
		int tIdx = int(vTerrainType + 0.5);
		vec3 tCol = tc[tIdx];

		float dfc = length(vHexUV);
		float eb = smoothstep(0.5, 0.95, dfc);
		...
		// Hex edge line
		float eDk = smoothstep(0.85,0.95,dfc) * 0.12;
		tCol *= (1.0 - eDk);

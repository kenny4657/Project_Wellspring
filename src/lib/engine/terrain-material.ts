...
void main() {
    ...
    if (isWall) {
        ...
        if (cliffProximity > 0.01 && steepness > 0.003) {
            ...
            // Proximity fill for midpoint gap — only at very high proximity
            // (right at the cliff edge where the two halves meet)
            float proxCover = smoothstep(0.7, 0.95, cliffProximity);
